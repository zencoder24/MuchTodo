...
const TodoList = ({ todos, error, fetching }) => {
  ...
  if (error) {
    return (
      <h2 className="text-xl text-gray-200">Error! Please try again later!</h2>
    );
  }
  if (!todos || !todos.length) {
    return <h2 className="text-gray-200 text-xl ">No Todos Yet!</h2>;
  }
  ...
  if (fetching) {
    return (
      <div role="status">
        <svg
          class="inline mr-2 w-10 h-10 text-gray-600 animate-spin  fill-blue-500"
          viewBox="0 0 100 101"
          fill="none"
